...
enum Term {
    ...
    extension (self: I) {
        @targetName("shiftI")
        def >>(amount: Int): I = self.map { case (key, value) => (key >> amount, value + 1) }
    }
    ...
    def check(shape: Term, p: P): Unit = this match
        case Term.Imp(t1, t2) if !shape.isInstanceOf[Imp] && query(t1, p, p) => t2.check(shape)
        case _ => check(shape)
    ...
    def search(typ: Term, i: I, all: I): Option[Term] = i.headOption match
        case Some(imp, index) if imp === typ => Some(Var(index))
        case Some((Imp(t1, t2), i)) => search(t1, all, all).map { arg => App(Var(i), arg) }
